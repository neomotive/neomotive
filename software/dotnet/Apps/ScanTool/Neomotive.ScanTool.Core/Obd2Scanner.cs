...
public class Obd2Scanner : IObd2Scanner
{
    ...
    public async Task<PidValue?> ReadPidAsync(Pid pid, CancellationToken ct = default)
    {
        ...
        double raw = descriptor.ByteCount == 1
            ? data[2]
            : (data[2] << 8) | data[3];

        return new PidValue(descriptor, raw * descriptor.Scale + descriptor.Offset, DateTime.UtcNow);
    }
