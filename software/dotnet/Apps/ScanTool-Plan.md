...
**Both appliances unified (2026-09-10):** the simulator now renders through Avalonia
DRM/KMS exactly as the ScanTool does — no X server, no xinit, no xinitrc. Its MainWindow
only ever wrapped a single SimulatorView, so the X stack was buying nothing while costing
a display stack the appliance image does not ship and a PrivateTmp drop-in to work around
ProtectSystem=strict. `run` and `setup-usb-updates.sh` are now near-identical between the
two apps. Bring-up from a blank SD card: `docs/commissioning-a-pi.md`.

**USB updates were broken on both devices and are now fixed (2026-09-10):** mounting from
a udev RUN rule cannot work — udev workers have a private mount namespace, so the mount is
refused or invisible. udev now pulls in a systemd template unit that mounts in the host
namespace; verified that the running app sees /media/usb through ProtectSystem=strict.
Group U in ScanTool-Tasks.md.
...
- Exercise a real USB update package end-to-end; only the mount is proven so far
